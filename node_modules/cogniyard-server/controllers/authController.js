...
exports.googleAuth = async (req, res, next) => {
  try {
    ...
    const googleRes = await axios.get('https://oauth2.googleapis.com/tokeninfo', { params: { id_token: googleToken }, timeout: 8000 });
    const { email, name, sub: googleId, picture: avatar, email_verified: emailVerified, aud } = googleRes.data;
    if (!email || String(emailVerified) !== 'true') {
    ...
    if (process.env.GOOGLE_CLIENT_ID && aud !== process.env.GOOGLE_CLIENT_ID) {
      return res.status(401).json({ success: false, message: 'Google token audience is invalid.' });
    }
    ...
    if (!user) {
      if (process.env.NODE_ENV === 'production' && process.env.ALLOW_PUBLIC_REGISTRATION !== 'true') {
        return res.status(403).json({
          success: false,
          message: 'This Google account has not been provisioned by an administrator.'
        });
      }
      user = new User({
