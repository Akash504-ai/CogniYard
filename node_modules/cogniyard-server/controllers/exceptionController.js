...
const syncExceptionsFromCollections = async () => {
  try {
    ...
    const mismatchInvoices = await Invoice.find({ matchStatus: { $in: ['MISMATCH', 'MISMATCHED', 'PARTIALLY_MATCHED'] } });
    for (const inv of mismatchInvoices) {
      ...
      if (!exists) {
        const reason = inv.matchDetails?.reasons?.[0] || inv.ocrData?.reasons?.[0] || 'Quantity or amount discrepancy against PO & Goods Receipt';
        const newEx = await Exception.create({
          type: 'INVOICE_MISMATCH',
          category: 'FINANCE',
          severity: 'CRITICAL',
          title: `3-Way Match Failed on Invoice ${inv.invoiceNumber}`,
          description: `Invoice ${inv.invoiceNumber} for PO ${inv.poNumber} failed 3-Way verification. Billed Total: ₹${inv.totalAmount.toLocaleString('en-IN')}. ${reason}`,
          sourceType: 'INVOICE',
          sourceId: inv._id.toString(),
          status: 'OPEN',
    ...
    for (const pay of onHoldPayments) {
      ...
      if (!exists) {
        const newEx = await Exception.create({
          ...
          description: `Payment ${pay.paymentNumber} for Invoice ${pay.invoiceNumber} (${pay.supplierName}) is locked ON_HOLD. Amount: ₹${pay.amount.toLocaleString('en-IN')}.`,
          sourceType: 'PAYMENT',
          sourceId: pay._id.toString(),
          status: 'OPEN',
    ...
    for (const pr of pendingPRs) {
      ...
      if (!exists) {
        ...
        const newEx = await Exception.create({
          ...
          description: `Purchase Requisition ${pr.prNumber} for ${itemName} (₹${pr.totalAmount.toLocaleString('en-IN')}) requires procurement authorization.`,
          sourceType: 'REQUISITION',
          sourceId: pr._id.toString(),
          status: 'OPEN',
