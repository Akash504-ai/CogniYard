...
export default function AIAssistantModal() {
  ...
  const toggleListening = () => {
    ...
    try {
      ...
      console.log('[SpeechRecognition Debug] Initialized instance', {
      ...
      recognition.onstart = () => {
        console.log('[SpeechRecognition Debug] onstart fired');
        setIsListening(true);
      };

      recognition.onresult = (event) => {
        console.log('[SpeechRecognition Debug] onresult fired', event.results);
        let interimText = '';
      ...
      recognition.onerror = (event) => {
        console.warn('[SpeechRecognition Debug] onerror fired', {
          error: event.error,
      ...
      recognition.onend = () => {
        console.log('[SpeechRecognition Debug] onend fired');
        setIsListening(false);
        recognitionRef.current = null;
      };

      recognitionRef.current = recognition;
      console.log('[SpeechRecognition Debug] Calling recognition.start()');
      recognition.start();
    } catch (err) {
      console.error('[SpeechRecognition Debug] Exception on start:', err);
      setIsListening(false);
      recognitionRef.current = null;
                ...
                {msg.requiresConfirmation && (
                  <div className="mt-3.5 pt-3 border-t border-amber-200/60 dark:border-amber-900/40 bg-amber-50/60 dark:bg-amber-950/30 p-3.5 rounded-xl space-y-2.5 text-xs">
                    ...
                    <div className="flex items-center justify-end gap-2 pt-1">
                      ...
                      <button
                        onClick={() => handleSend('Confirm Action', true, msg.params)}
                ...
                {msg.toolResult && msg.toolResult.success && (
                  <div className="mt-3.5 pt-3 border-t border-zinc-100 dark:border-zinc-800/80 space-y-2.5">
                    ...
                    {msg.toolResult.recommendedDock && (
                      <div className="bg-indigo-50/70 dark:bg-indigo-950/40 p-3 rounded-xl border border-indigo-200/60 dark:border-indigo-900/50 space-y-1">
                        <div className="flex justify-between items-center text-xs font-bold text-indigo-900 dark:text-indigo-200">
                          ...
                          <span className="font-mono bg-indigo-600 text-white text-[10px] px-2 py-0.5 rounded-md">
        ...
        <div className="p-4 border-t border-zinc-200/80 dark:border-zinc-800 bg-white dark:bg-zinc-900">
          ...
          >
            <input
              ...
              className={`w-full bg-zinc-50 dark:bg-zinc-950 border rounded-xl pl-4 pr-20 py-2.5 text-xs text-zinc-900 dark:text-zinc-100 placeholder:text-zinc-400 dark:placeholder:text-zinc-500 focus:outline-none focus:ring-1 focus:ring-indigo-500 transition-all font-sans ${
          ...
          <div className="mt-2 text-center">
            <span className="text-[10px] text-zinc-400 dark:text-zinc-600 font-mono">
              {isListening ? "🔴 Recording Active — Speech transcribing live..." : "CogniYard Copilot Engine • Voice-to-Text & RBAC Protected"}
            </span>
          </div>
        </div>
