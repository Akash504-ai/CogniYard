...
async function tick() {
  ...
  for (let truck of stateCache.activeTrucks) {
    ...
    if (truck.progress >= 95 && truck.status === 'UNLOADING') {
      ...
      if (truck.unloadingProgress >= 100) {
    ...
    } else if (truck.progress >= 60 && truck.status === 'AT_GATE') {
      truck.status = 'IN_YARD';
      truck.yardLocation = 'Checkpoint 2 (In Yard)';
      addEvent(`Truck ${truck.truckId} cleared Gate Checkpoint and entered Yard.`, 'info');
