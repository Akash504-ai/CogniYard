...
exports.getSuppliers = async (req, res, next) => {
  try {
    const suppliers = await Supplier.find().select('-bankDetails').sort({ rating: -1 });
...
exports.createPurchaseOrder = async (req, res, next) => {
  try {
    ...
    for (const item of itemsToProcess) {
      ...
      const rawUnitPrice = item.unitPrice ?? item.estimatedUnitPrice;
      const unitPrice = Number(rawUnitPrice);
      if (isNaN(unitPrice) || unitPrice <= 0) {
        return res.status(400).json({ success: false, message: `Invalid unit price for item '${item.productName}'. Must be greater than 0.` });
      }
    ...
    const taxAmount = Number((subtotal * safeTaxRate / 100).toFixed(2));
    ...
    if (!truck) {
      ...
      const truckSerial = truckId.replace(/\D/g, '');
